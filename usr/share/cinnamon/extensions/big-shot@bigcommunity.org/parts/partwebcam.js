...
try {
    _GstApp = imports.gi.GstApp;
} catch(e) {
    // GstApp not available
}
...
// =============================================================================
// Simple Icon+Label button helper (same pattern as partaudio.js)
// =============================================================================
...
var PartWebcam = class PartWebcam extends PartUI {
    ...
    _createButton() {
        let typeContainer = this._ui.controlContainer;
        if (!typeContainer)
            typeContainer = this._ui.panel;
        if (!typeContainer)
            return;

        let iconsDir = Gio.File.new_for_path(this._ext.path + '/data/icons');

        this._webcamButton = _createIconLabelButton(
        ...
        this._webcamButton.connect('notify::checked', () => {
            ...
            if (this._enabled)
                ...
                this.stopPreview();
            if (this._webcamToggledCallback)
                this._webcamToggledCallback(this._enabled);
        });

        typeContainer.add_child(this._webcamButton);

        // Simple tooltip (no Screenshot.Tooltip in Cinnamon)
        this._webcamTooltip = new St.Label({
            text: _('Webcam Overlay'),
            style: 'background: rgba(0,0,0,0.85); color: #ffffff; padding: 4px 8px; border-radius: 4px; font-size: 11px;',
            visible: false,
        });
        this._ui.actor.add_child(this._webcamTooltip);

        this._webcamButton.connect('enter-event', () => {
            this._showTooltip(this._webcamButton, this._webcamTooltip);
        });
        this._webcamButton.connect('leave-event', () => {
            this._webcamTooltip.visible = false;
        });
    }
    ...
    get selectedDevice() { return this._selectedDevice ? this._selectedDevice : null; }
    ...
    enumerateDevices() {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            let name = 'Camera ' + i;
            try {
                let [ok, contents] = GLib.file_get_contents(nameFile);
                if (ok) {
                    let decoder = new TextDecoder();
                    name = decoder.decode(contents).trim();
                }
            } catch(e) { /* ignore */ }
            ...
            cams.push({ device: dev, name: name, index: i });
        }
        return cams;
    }
    ...
    _updateContainerLayout() {
        ...
        this._container.set_size(sz.w, sz.h);
    ...
    _createOverlay() {
        ...
        this._ui.actor.add_child(this._container);
    }

    /** Move the overlay to Chrome (for recording, when UI hides). */
    reparentForRecording() {
        ...
        this._ui.actor.remove_child(this._container);
    ...
    reparentForPreview() {
        ...
        this._connectDragCapture(this._ui.actor);
    }
            ...
            for (let x = 0; x < w; x++) {
                ...
                if (d2 > 1.0) {
                    ...
                    if (d < 0.5)
                        ...
                        brightness = 1.0 - 0.85 * Math.pow((d - 0.5) / 0.5, 1.6);
    ...
    _hitTestContainer(stageX, stageY) {
        if (!this._container)
            return false;
        let result = this._container.transform_stage_point(stageX, stageY);
        if (!result)
            return false;
        let ok = result[0];
        let lx = result[1];
    ...
    _setupDrag() {
        ...
        // when the overlay is reparented to Chrome during recording.
        this._connectDragCapture(this._ui.actor);
    }
    ...
    _tryV4l2Device(device) {
        try {
            ...
            let PAUSED = 2;
            let FAILURE = 0;
            if (Gst.State && Gst.State.PAUSED !== undefined)
                PAUSED = Gst.State.PAUSED;
            if (Gst.StateChangeReturn && Gst.StateChangeReturn.FAILURE !== undefined)
                FAILURE = Gst.StateChangeReturn.FAILURE;

            const ret = pipeline.set_state(PAUSED);

            if (ret === FAILURE) {
                let NULL_STATE = 1;
                if (Gst.State && Gst.State.NULL !== undefined)
            ...
            let PLAYING = 4;
            if (Gst.State && Gst.State.PLAYING !== undefined)
                PLAYING = Gst.State.PLAYING;
            this._pipeline.set_state(PLAYING);
            return true;
        } catch(e) {
            return false;
        }
    ...
    _startPipeline() {
        ...
        if (!started) {
            log('[Big Shot Webcam] No usable webcam device found');
    ...
    _probeFrame(device) {
        ...
        if (elapsed > 5.0) {
            log('[Big Shot Webcam] Probe timed out — no frames received');
            this._stopPipeline();
            this._onWebcamFailed();
            return GLib.SOURCE_REMOVE;
        }
        ...
        let nwResult = struct.get_int('width');
        let nhResult = struct.get_int('height');
        let nw = nwResult[1];
        let nh = nhResult[1];
        ...
        if (Gst.State && Gst.State.NULL !== undefined)
            NULL_STATE = Gst.State.NULL;
        this._pipeline.set_state(NULL_STATE);
        this._pipeline = null;
        this._sink = null;
        this._probing = false;
    ...
    _startRenderPipeline(device, w, h, nativeW, nativeH) {
        try {
            ...
            const ret = this._pipeline.set_state(PLAYING);
            if (ret === FAILURE) {
                log('[Big Shot Webcam] Render pipeline failed to start');
                let NULL_STATE = 1;
                if (Gst.State && Gst.State.NULL !== undefined)
                    NULL_STATE = Gst.State.NULL;
                this._pipeline.set_state(NULL_STATE);
            ...
            log('[Big Shot Webcam] Pipeline failed: ' + e.message);
            this._pipeline = null;
            this._onWebcamFailed();
        }
    }
    ...
    _pollFrame() {
        ...
        try {
            ...
            let READ_FLAG = 1;
            if (Gst.MapFlags && Gst.MapFlags.READ !== undefined)
                READ_FLAG = Gst.MapFlags.READ;
            let mapResult = buffer.map(READ_FLAG);
            let success = mapResult[0];
            let mapInfo = mapResult[1];
            if (!success)
                return;
...
PartWebcam.setGettext = function(fn) { _ = fn; };
