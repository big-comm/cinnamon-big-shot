 ...
 * @param {number} h
 */
var paintGradient = function(cr, gradient, x, y, w, h) {
    ...
    for (let i = 0; i < gradient.stops.length; i++) {
        ...
        pat.addColorStopRGBA(stop[0], stop[1], stop[2], stop[3], 1.0);
    }
 ...
 */
var paintDropShadow = function(cr, x, y, w, h, radius, offsetY) {
    if (radius === undefined) radius = 16;
    if (offsetY === undefined) offsetY = 4;
 ...
 */
var paintRoundedRect = function(cr, x, y, w, h, r) {
    if (r <= 0) {
        cr.rectangle(x, y, w, h);
        return;
    }
